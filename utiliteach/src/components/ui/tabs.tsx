...
const Tabs = ({ links }: { links: { link: string, icon: React.JSX.Element, label: string }[] }) => {
  return (
    <div className={`w-full flex bg-white fixed bottom-0 border border-t-black h-24 max-sm:text-xs md:hidden ${window.location.pathname === '/teacher/dashboard' ? 'hidden' : ''}`}>
      {
